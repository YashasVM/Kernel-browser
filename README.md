...
<p align="center">
  <strong>Safari-inspired chrome &middot; Google by default &middot; private tabs &middot; bundled extensions</strong><br>
  <sub>Kernel Browser is an Android browser shell around Mozilla GeckoView with native Kotlin UI, square tab previews, auto-hiding bottom controls, history, settings, and allowlisted WebExtension support.</sub>
</p>

---

> [!IMPORTANT]
> Kernel Browser is still an early Android browser project. Use the ABI-specific APK for your device; modern Samsung/Pixel phones should install the `arm64-v8a` release.

## What Is Kernel Browser?

Kernel Browser is a native Android browser built with Kotlin, XML views, and `GeckoView`. It keeps the UI lightweight and Android-first while using Mozilla's browser engine instead of `WebView`.
...
| APK | Use On |
|---|---|
| `KernelBrowser-*-arm64-v8a.apk` | Modern phones, including Galaxy S24 Ultra |
| `KernelBrowser-*-armeabi-v7a.apk` | Older 32-bit Android devices |
| `KernelBrowser-*-x86_64.apk` | Android emulators and x86_64 devices |
...
& $adb install -r app\build\outputs\installable\KernelBrowser-0.1.9-polished-google-tabs-ui-2026-06-23-arm64-v8a.apk
```
...
- Add GitHub Actions release builds for ABI split APKs.
- Keep reducing UI inconsistency across settings, extensions, and tabs.
